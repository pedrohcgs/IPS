###################################################################################
# IPW estimator for the Quantile Treatment Effect on the Treated
#' IPW estimator for the Quantile Treatment Effect on the Treated
#'
...
#' @param tau An \eqn{l} x \eqn{1} vector of quantile to compute the QTT at. If NULL, then we set tau = 0.5.
...
#' \item{qtt}{The estimated QTT}
#' \item{qtt.se}{Estimated (pointwise) std. error of the QTT.}
#' \item{qtt.inf}{Estimated influence function of QTT estimator.}
#' \item{tau}{The evaluation points of QTT.}
#'
#' @references
...
QTT <- function(y, d, x, ps, beta.lin.rep, tau = 0.5, bw = "nrd0", 
  ...
  if(is.null(trim.at)) trim.at <- c(0, 1 - 1e-16)
  ...
  if((ps.d1.f>0) && (ps.d0.f>0)){
    ...
    # QTT                             
    qtt.hat <- q1.hat - q0.hat
    #-----------------------------------------------------------------------------
    # Compute influence function of QTT
    ...
    for (s in 1:st){
      ...
      q.Y1.inf[,s] <- - q.Y1.inf1[,s]/f1[s]
      q.Y0.inf[,s] <- -(q.Y0.inf1[,s] + beta.lin.rep %*% G0.beta[,s])/f0[s]
    }
    # Influence function of the qtt
    qtt.inf <- q.Y1.inf - q.Y0.inf   # n x st
    #-----------------------------------------------------------------------------
    # Compute standard error
    qtt.var <- base::diag(stats::cov(qtt.inf))
    qtt.se <- base::sqrt(qtt.var/n)
  } else {
    qtt.hat <- NA
    qtt.se <- NA
    qtt.inf <- NA
  ...
  #-----------------------------------------------------------------------------
  # Return qtt.hat, qtt.se, qtt.inf, and tau
  out <- list(qtt = qtt.hat,
              qtt.se = qtt.se,
              qtt.inf = qtt.inf,
              tau = tau)
  return(out)
