...
#' @param ysup An \eqn{l} x \eqn{1} vector of points in the support of y to compute the DTE at.
...
DTE <- function(y, d, x, ps, beta.lin.rep, ysup = NULL,
  ...
  if((ps.d1.f>0) && (ps.d0.f>0)){
    ...
    dte.hat <- F1.hat.est - F0.hat.est
    #-----------------------------------------------------------------------------
    # Compute influence function of DTE
    ...
    G1.beta <- base::crossprod(ps.dot.prime/ps,
                               cdf.Y1.inf1) / n           # k x n.ysup
    G0.beta <-  base::crossprod(ps.keep * ps.dot.prime/(1 - ps),
                                cdf.Y0.inf1) / n          # k x n.ysup
    
    # Estimations effect themselves
    cdf.Y1.est.eff <- beta.lin.rep %*% G1.beta
    cdf.Y0.est.eff <- beta.lin.rep %*% G0.beta
    
    # dte Influence function
    cdf.Y1.inf <- cdf.Y1.inf1 - cdf.Y1.est.eff
    cdf.Y0.inf <- cdf.Y0.inf1 + cdf.Y0.est.eff
    dte.inf <- cdf.Y1.inf - cdf.Y0.inf
    #-----------------------------------------------------------------------------
    # Compute standard error
    dte.var <- base::diag(stats::cov(dte.inf))
    dte.se <- base::sqrt(dte.var/n)
  } else {
    dte.hat <- NA
    dte.se <- NA
    dte.inf <- NA
