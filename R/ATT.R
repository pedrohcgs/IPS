###################################################################################
# IPW estimator for the Average Treatment Effect on the Treated
#' IPW estimator for the Average Treatment Effect on the Treated
#'
...
#' \item{att}{The estimated ATT}
#' \item{att.se}{Estimated std. error of the ATT.}
#' \item{att.inf}{Estimated influence function of ATT estimator.}
#'
#' @references
...
ATT <- function(y, d, x, ps, beta.lin.rep, trim = FALSE, 
                trim.at = NULL,
                whs = NULL){
  #-----------------------------------------------------------------------------
  ...
  if((ps.d1.f>0) && (ps.d0.f>0)){
    ...
    att.hat <- mu.Y1 - mu.Y0
    #-----------------------------------------------------------------------------
    # Compute influence function of ATT
    # Estimation effects
    ...
    mu.Y1.inf <- mu.Y1.inf1 
    mu.Y0.inf <- mu.Y0.inf1 + mu.Y0.est.eff
    att.inf <- mu.Y1.inf - mu.Y0.inf
    #-----------------------------------------------------------------------------
    # Compute standard error
    att.var <- stats::var(att.inf)
    att.se <- base::sqrt(att.var/n)
  }else {
    att.hat <- NA
    att.se <- NA
    att.inf <- NA
  ...
  #-----------------------------------------------------------------------------
  # Return att.hat, att.se, and att.inf
  out <- list(att = att.hat,
              att.se = att.se,
              att.inf = att.inf)
  return(out)
}
