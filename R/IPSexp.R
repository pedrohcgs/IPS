###################################################################################
# Integrated Propensity Score estimator based on exponential weighting function
#' Integrated Propensity Score estimator based on exponential weighting function
#'
...
#-------------------------------------------------------------------------------
IPS_exp = function(d, x, xbal = NULL, X.trans = "normal", Treated = FALSE,
  ...
  if(base::any(ps.hat<probs.min)) {
    base::message("IPS.exp: fitted probabilities smaller than 1e-10 occurred. We truncate these.")
  }
  if(base::any(ps.hat>(1-probs.min))) {
    base::message("IPS.exp: fitted probabilities bigger than 1 - 1e-10 occurred. We truncate these.")
  }
  ...
  if (lin.rep == TRUE){
   ...
   if(covSing==FALSE) base::message("IPS.exp: The variance-Covariance matrix is close to singular. Used Generalized-Inverse to compute std. errors.")
  }
